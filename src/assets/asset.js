import logo from "./elements/sixthLogoWhite.svg";
import Logo from "./elements/sixthLogo.svg";
import mail from "./elements/mail.png";
import mission from "../assets/Who/mission.png";
import vision from "../assets/Who/vision.png";
import whoBanner from "../assets/Who/whoBanner.webp";
import people from "../assets/Who/people.png";
import sustainability from "../assets/Who/sustainability.png";
import excellence from "../assets/Who/excellence.png";
import innovation from "../assets/Who/innovation.png";
import integrity from "../assets/Who/integrity.png";
import whoBanner2 from "../assets/Who/whoBanner2.webp";
import whoBanner3 from "../assets/Who/whoBanner3.webp";
import download from "../assets/Who/download.png";
import graphic from "../assets/Home/Graphic.png";
import home from "../assets/Home/home.webp";
import arrow from "../assets/Home/arrow.png";
import faculty1 from "../assets/Faculty/1.webp";
import faculty2 from "../assets/Faculty/2.svg";
import faculty3 from "../assets/Faculty/3.svg";
import faculty4 from "../assets/Faculty/4.svg";
import faculty5 from "../assets/Faculty/5.svg";
import faculty6 from "../assets/Faculty/6.svg";
import faculty7 from "../assets/Faculty/7.svg";
import faculty8 from "../assets/Faculty/8.svg";
import faculty9 from "../assets/Faculty/9.webp";
import faculty10 from "../assets/Faculty/10.svg";
import faculty11 from "../assets/Faculty/11.svg";
import faculty12 from "../assets/Faculty/12.svg";
import background from "../assets/Services/services.webp";
import donwloadB from "../assets/Services/downloadB.png";
import graphic1 from "../assets/Services/Graphic1.svg";
import graphic2 from "../assets/Services/Graphic2.svg";
import homeBg from "../assets/Home/HomeBg.webp";
import whoBg from "../assets/Home/WhoBg.webp";
import whatBg from "../assets/Home/WhatBg.webp";
import facultBg from "../assets/Home/FacultyBg.webp";
import contactBg from "../assets/Home/ContactBg.webp";
import blogBg from "../assets/Home/StoriesBg.webp";
import aboutBanner from "../assets/Who/homeBanner.webp";
import trainingBackground from "../assets/Services/trainingBackground.webp";
import retreatBackground from "../assets/Services/retreatBackground.webp";
import tlm from "../assets/Services/tlm.png";
import mp3 from "../assets/Services/mp3.png";
import bespoke from "../assets/What/bespoke.png";
import Assessments from "../assets/What/Assessments.png";
import Expert from "../assets/What/Expert.png";
import Case from "../assets/What/Case.png";
import Brainteasers from "../assets/What/Brainteasers.png";
import Creative from "../assets/What/Creative.png";
import Role from "../assets/What/Role.png";

export const assets = {
  background,
  aboutBanner,
  donwloadB,
  logo,
  Logo,
  mail,
  bespoke,
  mission,
  vision,
  whoBanner,
  sustainability,
  people,
  excellence,
  innovation,
  integrity,
  whoBanner2,
  download,
  graphic,
  tlm,
  mp3,
  home,
  arrow,
  graphic1,
  retreatBackground,
  graphic2,
  homeBg,
  whoBg,
  whatBg,
  facultBg,
  contactBg,
  blogBg,
  whoBanner3,
  trainingBackground,
};

export const coreValues = [
  {
    title: "people",
    icon: people,
    alt: "people icon",
    description: "We prize the people we work with and work for",
  },
  {
    title: "integrity",
    icon: integrity,
    alt: "integrity icon",
    description: "We uphold moral excellence",
  },
  {
    title: "Innovation",
    icon: innovation,
    alt: "innovation icon",
    description: "We create the future",
  },
  {
    title: "sustainability",
    icon: sustainability,
    alt: "sustainability icon",
    description: "We think long term",
  },
  {
    title: "excellence",
    icon: excellence,
    alt: "excellence icon",
    description: "We are always raising the bar to make our next work our best",
  },
];

export const facultyList = [
  {
    name: "BRIGHT UKWENGA MCloD, FIMC, CMC",
    image: faculty1,
    linkedin: "https://www.linkedin.com/in/brightuk/",
  },
  {
    name: "DEMOLA ADERIBIGBE",
    image: faculty2,
    linkedin: "https://www.linkedin.com/in/demola-aderibigbe-26484447//",
  },
  {
    name: "KUNLE AKIODE, Ph.D",
    image: faculty3,
    linkedin: "https://www.linkedin.com/in/kunleakiode/",
  },
  {
    name: "EIZU UWAOMA",
    image: faculty4,
    linkedin: "https://www.linkedin.com/in/eizu/",
  },
  {
    name: "GOSPEL OBELE",
    image: faculty5,
    linkedin: "https://www.linkedin.com/in/dr-gospel-obele-50b51252/",
  },
  {
    name: "LESLIE GEORGE",
    image: faculty6,
    linkedin: "https://www.linkedin.com/in/leslie-george-94581422/",
  },
  {
    name: "OLAWALE THOMAS",
    image: faculty7,
    linkedin: "https://www.linkedin.com/in/wale-thomas-3b10606a/",
  },
  {
    name: "TINUKEMI OLAOYE",
    image: faculty8,
    linkedin: "https://www.linkedin.com/in/tinukemiolaoye/",
  },
  {
    name: "GABRIEL BALOGUN",
    image: faculty9,
    linkedin: "https://www.linkedin.com/in/gabrielbalogun/",
  },
  {
    name: "ADERINSOLA JOLAOSHO",
    image: faculty10,
    linkedin: "https://www.linkedin.com/in/aderinsola-jolaosho/",
  },
  {
    name: "SOGO AKERELE",
    image: faculty11,
    linkedin:
      "https://www.linkedin.com/in/akerele-oluwasogo-glmp-mr-excel-11066710b/",
  },
  {
    name: "DAPO ONAMUSI",
    image: faculty12,
    linkedin: "https://www.linkedin.com/in/dapo-onamusi",
  },
];

export const approaches = [
  {
    icon: Assessments,
    title: "Assessments & Analyses",
    text: "Structured assessments to gouge current skill levels and target areas for improvement",
    color: "bg-[#172a4d]",
  },
  {
    icon: Expert,
    title: "Expert Insights",
    text: "Facilitator-led sessions by seasoned industry professionals, offering actionable strategies and leadership frameworks.",
    color: "bg-[#2a4e7e]",
  },
  {
    icon: Case,
    title: "Case Studies",
    text: "Real-world scenarios to apply concepts and enhance practical understanding",
    color: "bg-[#1b4d7e]",
  },
  {
    icon: Brainteasers,
    title: "Brainteasers",
    text: "Engaging puzzles to stimulate critical thinking and problem-solving abilities",
    color: "bg-[#4b6a92]",
  },
  {
    icon: Creative,
    title: "Creative Visuals",
    text: "Visual aids to illustrate complex ideas and support retention of key concepts",
    color: "bg-[#4e6b8e]",
  },
  {
    icon: Role,
    title: "Role Play & Team Activities",
    text: "Interactive exercises to build communication skills and encourage  collaborative learning.",
    color: "bg-[#6a9bc2]",
  },
];
