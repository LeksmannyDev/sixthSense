...
const ServiceSection = () => {
  return (
    <section className="bg-[#E2EDFF] py-16 px-4 sm:px-6 md:px-20">
      ...
      <div className="flex flex-wrap justify-center gap-6 lg:gap-x-4 gap-y-10">
        {services.map((service, index) => (
          <div
            key={index}
            className="bg-white rounded-4xl shadow-md overflow-hidden border-b-[8px] border-[#003366] w-full max-w-[450px]"
          >
            <img
              src={service.image}
              alt={service.title}
            ...
            <div className="px-4 sm:px-6 md:px-7 py-7 sm:py-9">
              <h3 className="text-2xl sm:text-3xl font-semibold text-deepBlue mb-4 md:mb-8 max-w-full sm:max-w-[50%]">
                {service.title}
              </h3>
                ...
                {service.items.map((item, idx) => {
                  ...
                  return (
                    <li key={idx}>
                      <ItemWrapper
                        {...wrapperProps}
                        className="flex items-center justify-between border border-[#003366] px-4 sm:px-6 md:px-7 py-3 sm:py-4 rounded-[15px]  hover:bg-[#E2EDFF] transition cursor-pointer"
                      >
